...
class YoloV3Loss(nn.Module):
    ...
    def __cal_loss_per_layer(self, p, p_d, label, bboxes, stride):
        ...
        # loss giou
        giou = utils.data_utils.GIOU_xywh_torch(p_d_xywh, label_xywh).unsqueeze(-1)
        ...
        # loss confidence
        iou = utils.data_utils.iou_xywh_torch(p_d_xywh.unsqueeze(4), bboxes.unsqueeze(1).unsqueeze(1).unsqueeze(1))
...
if __name__ == "__main__":
    ...
    loss, loss_xywh, loss_conf, loss_cls = YoloV3Loss(cfg.MODEL["ANCHORS"], cfg.MODEL["STRIDES"])(p, p_d, label_lbbox, label_mbbox,label_sbbox,
                                     lbboxes, mbboxes, sbboxes)
    print(loss)